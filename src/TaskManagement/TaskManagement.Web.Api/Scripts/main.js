...
function LoadUsers() {
    ...
    $.getJSON("/api/users", null,
            ...
            function Display(data) {
                ...
                var subItems = "<h3>Users</h3>" + "<ul>";

                $.each(data, function (index, item) {
                    subItems += "<li>" + item.nickname
                               + "</li>"
                });
...
function LoadUsersByTask(idTask) {

    $.getJSON("/api/task/" + idTask + "/users/", null,

            function Display(data) {

                var subItems = "<h4>Users</h4>" + "<ul>";
                ...
                subItems += "</ul>";
                $("#showItemMenuTaskUsers").html(subItems);
                CleanDetails();
            }
...
function GetTask(idTask) {
    ...
    $.getJSON("/api/Task/"+idTask, null,
            ...
            function Display(data) {
                ...
                subItems += '<p id="showItemMenuTaskUsers"><a href="javascript:void(0);" onclick="LoadUsersByTask(' + idTask + ')">Users</a></p>';
